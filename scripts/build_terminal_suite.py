...
def starship_palette_table(variant):
  options, ansi = ghostty_palette.load(ROOT / variant.ghostty_path)
  vscode = json.loads((ROOT / variant.vscode_path).read_text(encoding="utf-8"))
  ui = vscode["colors"]
  return f"""[palettes.{variant.starship_palette}]
background = "{options["background"]}"
foreground = "{options["foreground"]}"
surface = "{ui["editor.background"]}"
surface_deep = "{ui["activityBar.background"]}"
surface_raised = "{ui["editorWidget.background"]}"
border = "{ui["editorWidget.border"]}"
dim = "{variant.prompt_dim}"
amber = "{variant.prompt_amber}"
amber_bright = "{variant.prompt_amber_bright}"
red = "{ansi[1]}"
bright_red = "{ansi[9]}"
green = "{ansi[2]}"
...
def write_starship():
  default_palette = VARIANTS[0].starship_palette
  palettes = "\n".join(starship_palette_table(variant) for variant in VARIANTS)
  starship = f"""# Amber Material High Contrast — Starship prompt
# Generated by scripts/build_terminal_suite.py; edit the source palettes instead.
# Both Ghostty variants live here. Switch with:
#   palette = "amber_material"        # dark
#   palette = "amber_material_light"  # light
